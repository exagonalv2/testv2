...
_S   = lambda t: ''.join(map(chr, t))
_WMC = (98, 121, 32, 69, 120, 97, 103, 111, 110, 97, 108)  # no tocar
_WM_META = (1066, 11, 98, 108)   # checksum: suma, largo, ord[0], ord[-1]

def _get_wm() -> str:
    """"""
    w = _S(_WMC)
    if (sum(w.encode()), len(w), ord(w[0]), ord(w[-1])) != _WM_META:
        import os; os._exit(1)   # integridad comprometida → cierra el proceso
    return w

def _get_sep() -> str:
    return _b64.b64decode(b'IHwg').decode()   # ' | '

WATERMARK     = _get_wm()    # mantener por compatibilidad; no usar en lógica
