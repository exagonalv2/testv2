...
embed.add_field(name=‘📚 discord.py’, value=discord.**version**, inline=True)
...
def **init**(self, pages: list, author_id: int):
...
embed.set_image(url=gif)
await i.response.send_message(embed=embed)
*slash_anime.**name** = f’slash*{accion}’
...
“”“Hace que el bot diga un mensaje en el canal actual.”””
try:
await ctx.message.delete()
except Exception:
pass
...
@bot.tree.command(name=‘say’, description=‘Hace que el bot envíe un mensaje en el canal’)
@app_commands.describe(mensaje=‘Mensaje a enviar’)
async def slash_say(i: discord.Interaction, mensaje: str):
if not _is_staff(i):
return await _no_perm(i)
await i.response.send_message(‘✅ Mensaje enviado.’, ephemeral=True)
await i.channel.send(mensaje)
